...
class CinemaOneScreen extends StatefulWidget {
  final String spotName;
  final int id;

  CinemaOneScreen({required this.id, required this.spotName});

  @override
  _CinemaOneScreenState createState() => _CinemaOneScreenState();
}

class _CinemaOneScreenState extends State<CinemaOneScreen> {
  final Dio _dio = Dio();
  ...
  Future<void> _getShow() async {
    ...
    final response =
        await _dio.get("http://localhost:9000/api/show/showAll/${widget.id}");
    ...
    _dio.options.headers["Authorization"] = "Bearer $_token";
    final roomsResponse =
        await _dio.get("http://localhost:9000/api/rooms/roomsAll/${widget.id}");
  ...
  void _showReservation(timeItem, name) {
    showCupertinoDialog(
        context: context,
        builder: (BuildContext context) {
          return CupertinoAlertDialog(
              title: Text(timeItem),
              content: Text('예매 페이지로 이동! ${widget.spotName} 엔드 ${name}'),
              actions: [
                  ...
                  ? Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final show = _show[index];

                          return CupertinoListTile(
                            title: FutureBuilder(
                              future: _getTest(show['filePath']),
                                ...
                                if (test.connectionState ==
                                  ...
                                  return Column(
                                    ...
                                    children: [
                                      ...
                                      SizedBox(height: 8), // 이미지와 제목 사이의 간격 설정
                                      RichText(
                                        text: TextSpan(
                                          text: '[${show['age']}] ',
                                      ...
                                      ListView.builder(
                                        shrinkWrap:
                                            true, // 이 옵션을 사용하여 내부 ListView 높이 조정
                                        physics:
                                            NeverScrollableScrollPhysics(), // 중첩된 스크롤 비활성화
                                        itemCount: _time.length, // 중첩된 항목 수
                                        itemBuilder: (context, index) {
                                          ...
                                          if (show['movieId'] ==
                                                  ...
                                                  return GestureDetector(
                                                    ...
                                                    child: Container(
                                                      padding: EdgeInsets.symmetric(
                                                          horizontal: 8.0,
                                                          vertical:
                                                              4.0), // Add padding inside the box
                                                      margin: EdgeInsets.symmetric(
                                                          horizontal:
                                                              8.0), // Optional: Adds space between each item
                                                      decoration: BoxDecoration(
                                                        color: CupertinoColors
                                                            .systemBrown, // Background color of the box
                                                        borderRadius:
                                            ...
                                            return SizedBox
                                                .shrink(); // 조건이 맞지 않으면 빈 공간을 차지하지 않도록 처리
                                          }
                                        },
                                      )
                                    ],
